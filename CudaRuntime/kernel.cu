...
__global__ void CUDAConversion(unsigned char* src, unsigned char* dst, int rows, int cols, int channels)
{
...
__global__ void CUDAConversion_GpuMat(PtrStep<byte> src, PtrStep<byte> dst, int rows, int cols, int channels)
{
    int col = threadIdx.x + blockIdx.x * blockDim.x;
    int row = threadIdx.y + blockIdx.y * blockDim.y;
    ...
    if (col < cols && row < rows)
    {
		int rgb_offset = (row * src.step + channels * col);
		dst[rgb_offset + 0] = src[rgb_offset + 2];
		dst[rgb_offset + 1] = src[rgb_offset + 1];
		dst[rgb_offset + 2] = src[rgb_offset + 0];
...
void ColorBGR2RGB(unsigned char* h_image, int height, int width, int channels) 
{
	...
	const dim3 dimGrid(((int)ceil((width) / 16) + 1), ((int)ceil((height) / 16)+ 1));
	const dim3 dimBlock(16, 16);
	CUDAConversion <<<dimGrid, dimBlock>>> (d_bgr_image, d_rgb_image, height, width, channels);
...
void ColorBGR2RGB_GpuMat(PtrStep<byte> src, PtrStep<byte> dst, int height, int width, int channels)
{
	...
	CUDAConversion_GpuMat << <dimGrid, dimBlock >> > (src, dst, height, width, channels);
}
